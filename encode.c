 ...
 * Verify source image, secret file and output image extensions.
 */
Status read_and_validate_encode_args(char *argv[], EncodeInfo *encInfo)
{
    ...
    // Validate output image or assign default name
    if(argv[4]==NULL)
    {
...
/*
 * Open source image, secret file
 * and destination image files.
 */
...
/*
 * Check whether the source image
 * has enough capacity to store data.
 */
...
{    
    ...
    int required_size=(54+16+32+32+32+encInfo->size_secret_file*8);   // Calculate required bytes
...
Status copy_bmp_header(FILE *fptr_src_image, FILE *fptr_dest_image)
{
    ...
    fread(imageBuffer,54,1,fptr_src_image);   // Read BMP header

    fwrite(imageBuffer,54,1,fptr_dest_image);   // Read BMP header
...
Status encode_secret_file_extn_size(int size, EncodeInfo *encInfo)
{
    ...
    long dest = ftell(encInfo->fptr_dest_image);  //Check offeset position and compare.
...
/*
 * Encode one byte into
 * image LSBs.
 */
Status encode_byte_to_lsb(char data, char *image_buffer)
{
   for(int i=0;i<8;i++)  // Store every bit into image LSB
   {
       int bit=(data>>(7-i)&1);
